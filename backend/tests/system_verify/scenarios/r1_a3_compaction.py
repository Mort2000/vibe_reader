"""R1 A3: Compaction coverage happy path (stub and real LLM)."""
...
from ..flows.audit import (
    export_a3_comment_audit,
    export_a3_compaction_audit,
...
from ..flows.metrics import budget_check_a3_step
...
async def run(
    ...
    """R1 A3: compaction with large read batches and post-compaction windows."""
...
async def _step_export_compaction_audit(ctx: ScenarioContext) -> None:
    await export_a3_compaction_audit(
        ctx,
        scenario_id=SCENARIO_ID,
        step_id="export_compaction_audit",
    )


async def _step_export_comment_audit(ctx: ScenarioContext) -> None:
...
async def _step_budget_check(ctx: ScenarioContext) -> None:
    await budget_check_a3_step(ctx, scenario_id=SCENARIO_ID, step_id="budget_check")
